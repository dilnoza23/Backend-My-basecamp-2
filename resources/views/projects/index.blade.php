...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-gray-800 text-gray-200 leading-tight">
            {{ __('Projects') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-1">
            <div class="bg-white bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 text-gray-100">
                    ...
                    <div class="mt-4">
                        <h3 class="text-lg font-semibold">Name: {{ $project->name }} - <span> project members:
                            @if (!empty($project->admins()))
                                ( {{ $project->admins()->count() + 1 }} )</span></h3>
                            @else 
                                    1</span></h3>                                
                            @endif
                                
                        <p class="text-gray-600 text-gray-400">Description: {{ $project->description }}</p>
                    </div>
    ...
    @if ($project->admins()->where('user_id', auth()->user()->id)->exists() || $project->user_id === auth()->user()->id)
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 text-gray-100">
                    <form method="POST" action="{{ route('projects.addAdmin', $project->id) }}">
                        @csrf
                        <div class="form-group">
                            <label for="admin_user_id" class="text-lg font-semibold" style="margin-bottom: 10px;">Assign
                                Admin:</label>
                            <div class="flex">
                                <select name="user_id" id="user_id" class="form-control" class="form-control m-3"
                                    style="height: 50px; width: 90%; color: black;">
                                    <option value="">Select Admin User</option>
                                    @php
                                        $adminUserIds = $project->admins->pluck('user_id')->toArray();
                                        $projectOwnerId = $project->user_id;
                                    @endphp

                                    @foreach ($users as $user)
                                        @if ($user->id !== $projectOwnerId && !in_array($user->id, $adminUserIds))
                                ...
                                <div class="input-group-append">
                                    <button type="submit" class="btn-md ml-3 border rounded"
                        ...
                        <ul class="mt-4 flex">
                            @forelse ($project->attachments as $attachment)
                                <li class="py-2">
                                    ...
                                    <div class="flex mt-2" style="justify-content: space-around;">
                                        @if ($attachment->user_id === auth()->user()->id)
                                            <form action="{{ route('attachments.destroy', $attachment->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    style="margin-top:4px; padding-left:4px; border: 1px solid rgb(255, 55, 0); border-radius: 10px; width: 45px; background-color: #ff1e31; margin-right: 5px;">
                                        ...
                                        <a href="{{ route('attachments.download', $attachment->id) }}"
                                            style="text-decoration: none; padding-left: 5px; height: 38px; margin-top: 4px; border: 1px solid blue; border-radius: 10px; width: 45px; margin-right: 5px; display:block;">
                                            <svg width="35px" height="35px" viewBox="0 0 1024 1024" class="icon"
                                    ...
                                    <span class="text-gray-500 text-gray-300 ml-2">
                ...
                <div class="p-6 text-gray-900 text-gray-100">
                    <div class="flex" style="justify-content: space-between">
                        ...
                        @if ($project->user_id === auth()->user()->id)
                            <a href="{{ route('threads.index', $project) }}" class="add-project-link">
                                <button type="button" class="btn">📂 Add New Thread</button>
                            </a>
                        @endif
                    </div>
                    <div class="projects">
                        @if (!empty($project->threads))
                            @foreach ($project->threads as $thread)
                                <a href="{{ url('/thread/' . $thread->id . '/show') }}"
                                    class="project-link">
                                    <div class="project">
